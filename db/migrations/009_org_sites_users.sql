...
-- Bootstrap default org + site and backfill existing rows
DO $$
DECLARE
...
BEGIN
    ...
    VALUES (org_id, 'Default Company', NOW(), NOW())
    ON CONFLICT (id) DO NOTHING;

    INSERT INTO sites (id, organization_id, name, address, created_at, updated_at)
    VALUES (site_id, org_id, 'Основной склад', '', NOW(), NOW())
    ON CONFLICT (id) DO NOTHING;

    UPDATE spools SET site_id = site_id WHERE site_id IS NULL;
    UPDATE printers SET site_id = site_id WHERE site_id IS NULL;
    UPDATE print_jobs SET site_id = site_id WHERE site_id IS NULL;
    UPDATE inventory_transactions SET site_id = site_id WHERE site_id IS NULL;
    UPDATE bambu_cloud_accounts SET site_id = site_id WHERE site_id IS NULL;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'spools_site_id_fkey'
    ) THEN
        ALTER TABLE spools ADD CONSTRAINT spools_site_id_fkey
            FOREIGN KEY (site_id) REFERENCES sites(id);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'printers_site_id_fkey'
    ) THEN
        ALTER TABLE printers ADD CONSTRAINT printers_site_id_fkey
            FOREIGN KEY (site_id) REFERENCES sites(id);
    END IF;
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint WHERE conname = 'print_jobs_site_id_fkey'
    ) THEN
        ALTER TABLE print_jobs ADD CONSTRAINT print_jobs_site_id_fkey
            FOREIGN KEY (site_id) REFERENCES sites(id);
    END IF;
END $$;
